...
public class ValidateDepartmentNameFromEmployerCommand implements Command {
    ...
    public String execute(HttpServletRequest req, HttpServletResponse resp, ActionType actionType) throws IOException, ServletException, DAOException, SQLException {
        ...
        Employer employer = (Employer) dao.getEmployerByEmail((String) session.getAttribute("employerEmail"));

        dep1.setName(req.getParameter("DepName"));

        req.setAttribute("isNameExist", false);
        req.setAttribute("emailMessage", Constants.DEPARTMENT_DOES_NOT_EXIST_ERROR);
